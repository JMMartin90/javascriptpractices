...
const filterPokemonByType = (pokeList, type, type2) => {
  const pokemonByType = pokeList.filter((p) => {
    return (
      p.types.find((pt) => pt.type.name === type) && // cambiando el  "AND"&& o el "OR"|| podemos elejir coger de la lista pokemon que compartan esos tipos o que tengan ambos.
      (type2
        ? p.types.find((pt) => pt.type.name === type2) // ? es igual a "if" y : es igual a "else"
        : true)
    );
  });
